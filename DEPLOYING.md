...
```shell
docker image build -t lsssui .
docker run -it lsssui
```

<sub>_* Note: `-t lsssui` and `-it lsssui` may be changed to another tag name as desired, such as `-t developing_on_this` and `-it developing_on_this`._</sub><br>
